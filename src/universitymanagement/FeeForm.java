...
public class FeeForm extends JFrame implements ActionListener, ItemListener {
	...
	public FeeForm() {
		 ...
		 * 
		 * try { Connec c = new Connec(); ResultSet rs =
		 * c.s.executeQuery("select * from student where rollno = '" + ie.getItem() +
		 * "'"); while (rs.next()) { t2.setText(rs.getString("name"));
		 * t3.setText(rs.getString("fathers_name")); } } catch (Exception e) { } } });
		 */
	...
	public void actionPerformed(ActionEvent ae) {
		try {
			...
			if (ae.getSource() == b1) {
				if (c1.getSelectedItem().equals(null) || comboBox_3.getSelectedItem().equals(null)
						|| comboBox.getSelectedItem().equals(null) || comboBox_2.getSelectedItem().equals(null)
						|| t2.getText().equals("") || t3.getText().equals("") || t1.getText().equals("")) {
					JOptionPane.showMessageDialog(null, "Enter all the fields");
				} else {
					try {
						Connec con = new Connec();
						String sql = "insert into fee(rollno, name, fathers_name, course, branch, semester, fee_amount) values(?, ?, ?, ?, ?, ?, ?)";
	...
	public void itemStateChanged(ItemEvent ie) {
		if (ie.getSource() == c1)
			try {
				Connec c = new Connec();
				ResultSet rs = c.s.executeQuery("select * from student where rollno = '" + ie.getItem() + "'");
				while (rs.next()) {
